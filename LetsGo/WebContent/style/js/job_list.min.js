...
$(function () {
  $('.job_del').bind('click', function () {
    ...
    return confirm('确定要删除该职位吗？') ? ($.ajax({
      ...
      true==b ? top.location.reload()  : alert(b)      //      1 == $('.my_jobs li').length ? top.location.reload()  
      //    		  	: ($('#positionNumber').text($('#positionNumber').text() - 1), 
      //    		  	a.parents('li').remove())  : alert(b.msg)
      //
    }), void 0)  : !1
  }),
  
  $('.job_online').click(function (a) {
	    var b,
	    c,
	    d;
	    ...
	    d = $('#resubmitToken').val(),
	    confirm('确定要将该职位上线吗？上线后该职位将重新展现给用户') ? ($.ajax({
	      url: 'position?op=on',
	      data: {
	        pid: c,
	        resubmitToken: d
	      ...
	      dataType: 'json'
	    	...
	    	//alert("");
	      $('#resubmitToken').val(a.resubmitToken),
	      true==a ? 1 == $('.my_jobs li').length ? top.location.reload()  : ($('#positionNumber').text($('#positionNumber').text() - 1), b.parents('li').remove())  : alert('下线失败')
	    }), void 0)  : !1
	  }),
