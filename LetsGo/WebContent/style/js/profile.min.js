...
!function (a, b) {
  ...
  j.hsla.from = function (a) {
    ...
    h = 2 * d - g;
    return [Math.round(255 * e(h, g, b + 1 / 3)),
...
$(function () {
    ...
    $('.projectForm').each(function () {
      $(this).validate({
        ...
        submitHandler: function (a) {
          ...
          j = $('.projectId', a).val(),
          uid=$('input[name="id"]',a).val();
          l = $('#resumeId').val(),
          m = $('#resubmitToken').val();
          $(a).find(':submit').val('保存中...').attr('disabled', !0),
          $.ajax({
            url: 'resume?op=addPexp',
            type: 'POST',
            data: {
              uid: uid,
              pname: b,
  ...
  $('#profileForm').validate({
    ...
    submitHandler: function (a) {
      ...
      $.ajax({
        ...
        //$('#span').html("20");
        //$('#span').attr(20);
        /*b.success ? '/jianli.jsp' == b.content.url ?
        window.location.href = ctx + b.content.url : (c = b.content.resume, d = c.workYear, 
        '应届毕业生' != d ? (d += '工作经验', $('#workExperience h2 span').text('（投递简历时必填）'))  : $('#workExperience h2 span').text('')
        , e = c.name, '' != c.sex && (e += ' | ' + c.sex), '' != c.highestEducation && (e += ' | ' + c.highestEducation), e += ' | ' + d + '<br />' + c.phone + ' | ' + c.email + '<br />' + c.status,*/
        ...
        //$('#basicInfo .basicShow span').html(b.uname+""+b.usex+" "+b.uworkexp+'<br>'+b.utel+" "+b.uaccount);
        //$('#nameVal').val(c.name), $('#genderVal').val(c.sex), 
        //$('#topDegreeVal').val(c.highestEducation), 
        //$('#workyearVal').val(c.workYear), $('#currentStateVal').val(c.status), 
        //$('#emailVal').val(c.email), $('#telVal').val(c.phone),
        //$('#lastChangedTime span').text(b.content.refreshTime), 
        //b.content.isNew && changeAllIds(b.content.jsonIds), 
        //scoreChange(b.content.infoCompleteStatus.score), k($('#basicInfo')))  : alert(b.msg),
        $(a).find(':submit').val('保 存').attr('disabled', !1)
      })
    }
  }),
  ...
  $('#expectForm').validate({
    ...
    submitHandler: function (a) {
      ...
      g = $('input[name="id"]',a).val(),
      h = $('#resubmitToken').val();
      $(a).find(':submit').val('保存中...').attr('disabled', !0),
      $.ajax({
        url: 'resume?op=addWjob',
        type: 'POST',
        data: {
          uid: g,
          wcity: c,
  ...
  $('.box_companyYearEnd').on('click', 'ul li', function (a) {
    ...
    '至今' == $.trim(b) ? (d.find('.companyMonthEnd').val('至今'), d.find('.select_companyMonthEnd').css('color', '#333').val('至今'), d.find('.select_companyMonthEnd').unbind('click'))  : ('至今' == $.trim(c) && (d.find('.companyMonthEnd').val(''), d.find('.select_companyMonthEnd').css('color', '#999').val('结束时间')), d.find('.select_companyMonthEnd').bind('click', function (a) {
      a.stopPropagation(),
  ...
  $('#projectExperience').on('click', '.box_projectYearEnd li', function (a) {
    ...
    '至今' == $.trim(b) ? (d.find('.projectMonthEnd').val('至今'), d.find('.select_projectMonthEnd').css('color', '#333').val('至今'), $('#projectExperience').off('click', '.select_projectMonthEnd'))  : ('至今' == $.trim(c) && (d.find('.projectMonthEnd').val(''), d.find('.select_projectMonthEnd').css('color', '#999').val('结束时间')), $('#projectExperience').on('click', '.select_projectMonthEnd', function (a) {
      a.stopPropagation(),
      $('.profile_select_normal').removeClass('select_focus'),
      $('.boxUpDown').hide(),
      $(this).addClass('select_focus'),
  ...
  $('#resumeSetForm .btn_profile_cancel').bind('click', function () {
    ...
    - 1 == c ? (b.children('.noSet').show(), b.children('.set0').hide(), b.children('.set1').hide(), b.children('#resumeSetForm').addClass('dn'))  : (b.children('.noSet').hide(), b.children('.set' + c).show(), b.children('#resumeSetForm').addClass('dn')),
    a = !0
  }),
