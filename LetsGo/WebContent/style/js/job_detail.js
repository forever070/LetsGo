...
$(function(){
	...
	$('#resumeSendForm').validate({
    	...
    	submitHandler:function(form){
    		...
    		$(form).find(":submit").attr("disabled", true);
    		$.ajax({
    			url: 'resume?op=deliverResume',
    			data:{
    				cid:cid,//公司id
    				uid:userid,//用户id
    				pid:jobid,//职位id
    				rform:resumeName,//简历形式
	...
	$.colorbox({
		html:'<div class="popup" style="width:460px;">'+
				'<table width="100%">'+
					...
					'</tr>'+
					'<tr>'+
						'<td align="center"><a href="'+ctx+'/jianli.jsp" target="_blank" class="btn_s">马上去完善</a></td>'+
					'</tr>'+
				'</table>'+
			'</div>',
		title:'错误提示'
...
//投递成功弹窗
function successTipsSet(msg){
	$.colorbox({
		html:'<div class="popup" style="width:460px;">'+
		'<table width="100%">'+
		'<tr>'+
		'<td align="center"><h4 class="success_msg" style="width:400px;">'+msg+'</h4></td>'+
		'</tr>'+
		'</table>'+
		'</div>',
		title:'投递成功'
