...
$(function () {
  ...
  $('.resume_del').bind('click', function () {
    var a = $(this),
    p = a.parents('li').attr('pid'),
    b = a.parents('li').attr('uid'),
    c = $('#resubmitToken').val();
    return confirm('确定要删除该简历吗？') ? ($.ajax({
      type: 'POST',
      url: 'rmanage?op=updateByUid&rstate=0',
      data: {
    	pid:p,
        uid: b,
        cid: c
      ...
      true==a ? top.location.href = top.location.href : alert("删除失败！！！")
    }), void 0)  : !1
  }),
  //删除全部简历
  $('#resumeDelAll').bind('click', function () {
    ...
    return $('.resumeLists li').each(function () {
      $(this).find('input[type="checkbox"]').attr('checked') && a.push($(this).attr('uid'))
      $(this).find('input[type="checkbox"]').attr('checked') && p.push($(this).attr('pid'))
    }),
    //a = a.join(','),
  ...
  $('.resume_refuse').bind('click', function () {
    ...
    e=$(this).attr('data-email');
    b = $('#resubmitToken').val();
    p = $(this).attr('pid');
    $('#refuseMailForm input[name="deliverId"]').val(a),
    $('#refuseMailForm input[name="email"]').val(e),
    $('input[name="dhfgsu"]').val('发送'),
    $.ajax({
      url: 'rmanage?op=updateByUid&rstate=5',
      dataType: 'json',
      type: 'POST',
      async: !1,
      data: {
      ...
      true==a ? (
    		  //($('#refuseMailForm textarea').val().removeClass('error').removeClass('valid'), 
  ...
  $('#refuseMailForm').validate({
    ...
    submitHandler: function (a) {
      var e = $('input[name="email"]', a).val(),
      b = $('textarea[name="content"]', a).val(),
      c = $('input[name="deliverId"]', a).val(),
      d = $('#resubmitToken').val();
      $('input[name="dhfgsu"]').val('发送中');
      $.ajax({
        url: 'rmanage?op=refuseEmail',
        type: 'POST',
        data: {
          email:e,
          content: b,
  ...
  $('.resume_caninterview').click(function () {
    ...
    $.ajax({
      type: 'POST',
      url: 'rmanage?op=updateByUid&rstate=2',
      data: {
    	  pid:p,
        uid: a,
        cid: b
  ...
  $('.resume_forward').bind('click', function () {
    ...
    d = '（简历来自拉勾）'+ $(this).attr('data-resumeName'),
    e = '以下是应聘“' + $(this).attr('data-positionName') + '”的'+$(this).attr('data-resumeName')+'。我已查阅，请您评估一下。 若觉合适，我们将安排面试，谢谢！',
    $('input[name="dhfgsu"]').val('发送'),
  ...
  $('.resume_notice').bind('click', function () {
    ...
    $.ajax({
      type: 'POST',
      url: 'rmanage?op=updateByUid&rstate=4',
      data: {
        email: e,
        positionId: b,
        name: c,
      ...
      dataType: 'json'
    	...
    	true==a ? ($('#receiveEmail').text(e), //a.content.email
    			$('#noticeInterview input[name="email"]').val(e), //a.content.email
    			$('#noticeInterview input[name="name"]').val(c), //a.content.name
    			$('#noticeInterview input[name="subject"]').val("拉勾网通知面试").removeClass('error').removeClass('valid'), //a.content.subject
    			$('#noticeInterview input[name="interAdd"]').val("").removeClass('error').removeClass('valid'), //a.content.interAdd
    			$('#noticeInterview input[name="linkPhone"]').val("").removeClass('error').removeClass('valid'), //a.content.linkPhone
    			$('#noticeInterview input[name="linkMan"]').val(""), //a.content.linkMan
    			$('#noticeInterview textarea[name="content"]').val("祝你生活愉快"), //a.content.content
    			$('#noticeInterview input[name="positionName"]').val("positionName"), //a.content.positionName
    			$('#noticeInterview input[name="companyName"]').val("companyName"), //a.content.companyName
