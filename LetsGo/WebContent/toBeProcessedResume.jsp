...
<div id="body">
    ...
    <div id="container">
		...
		<dt>我收到的简历</dt>
		<dd class="current">
		<a href="rmanage?op=queryByCid&cid=${cid }&rstate=1">待处理简历</a> 
		
			</dd>
	<dd>
		<a href="rmanage?op=queryByCid&cid=${cid }&rstate=2">待定简历</a>
	</dd>
	<dd>
		<a href="rmanage?op=queryByCid&cid=${cid }&rstate=4">已通知面试简历</a>
	</dd>
	<dd>
		<a href="rmanage?op=queryByCid&cid=${cid }&rstate=5">不合适简历</a>
	</dd>
	<dd class="btm">
		<a href="rmanage?op=queryByCid&cid=${cid }&rstate=3">自动过滤简历</a> 
		<span>${rstate3 }</span>
			</dd>
</dl>
<dl class="company_center_aside">
		<dt>我发布的职位</dt>
			<dd>
		<a href="position?op=list">有效职位</a>
	</dd>
	<dd>
		<a href="position?op=listoff">已下线职位</a>
	</dd>
	</dl>
            </div><!-- end .sidebar -->
            <div class="content">
 ...
 <ul class="reset resumeLists">
       <li uid="${r.uid }" pid="${r.pid }" class="onlineResume">
             <label class="checkbox">
                 <input type="checkbox" name="uid" >
                  <i></i>
              </label>
               <div class="resumeShow">
               <a title="预览在线简历" target="_blank" class="resumeImg" href="resume?op=queryByUid&uid=${r.uid }">
                   <img src="style/images/default_headpic.png">
                   </a>
                   	<div class="resumeIntro">
                       <h3 class="unread">
                      <a target="_blank" title="预览${r.uname }的简历" href="resume?op=queryByUid&uid=${r.uid }">${r.uname }的简历</a>
                     <em></em>
                       </h3>
                       <span class="fr">投递时间：${r.ddate }</span>
                       <div> 
                       	${r.uname }/${r.usex }/${r.udegree }/${r.uworkexp }/${r.wcity }<br>
                           	${r.position }·${r.cno }|${r.udegree }·${r.sname }
                       </div>
                       <div class="jdpublisher">
                           <span>
                           	应聘职位：<a title="${r.pname }" target="_blank" href="position?op=listByPid&pid=${r.pid }&cid=${cid}">${r.pname }</a>
                           </span>
                       </div>
                   </div>
                   <div class="links">
                      	<a data-deliverid="${r.uid }" pid="${r.pid }" cid="${r.cid}" data-name="${r.uname }" data-positionid="149594" data-email="${r.uaccount }" class="resume_notice" href="javascript:void(0)">通知面试</a>
						<a data-deliverid="${r.uid }" pid="${r.pid }" data-email="${r.uaccount }" class="resume_refuse" href="javascript:void(0)">不合适</a>
						<a data-deliverid="${r.uid }" pid="${r.pid }" class="resume_caninterview" href="javascript:void(0)">待定</a>
                      <a data-resumename="${r.uname }的简历" pid="${r.pid }" data-positionname="${r.wposition }" data-deliverid="${r.uid }" data-positionid="${r.pid }" data-resumekey="1ccca806e13637f7b1a4560f80f08057" data-forwardcount="1" class="resume_forward" href="javascript:void(0)">
                       	转发
                        	<span></span>
                       </a>
                  </div>
              ...
              <div class="contactInfo">
			    <span class="c9">电话：</span>${r.utel }&nbsp;&nbsp;&nbsp;   
			    <span class="c9">邮箱：</span><a href="mailto:${r.uaccount }">${r.uaccount }</a>
			  </div>
         </li>
    </ul>
    </c:forEach>
	    ...
	    <form id="noticeInterviewForm">
	     	<table width="100%" class="f16">
	            ...
	            <tr>
	            	<td></td>
	             	<td>
	             		<input type="submit" value="发送" class="btn" name="dhfgsu">
...
再次感谢您对我们的信任，祝您早日找到满意的工作。</textarea>
	            ...
	            <tr>
	             	<td>
	             		<input type="submit" value="确认不合适" class="btn" name="dhfgsu">
	             		<a class="emial_cancel" href="javascript:;">取消</a>
	             	</td>
	            </tr>
	        </tbody></table>
...
<script>
			...
			<div class="clear"></div>
			<input type="hidden" value="${cid }" id="resubmitToken">
