	...
	<div id="header">
    	<div class="wrapper">
    		...
    		<ul class="reset" id="navheader">
    			...
    			<c:if test="${loginname.authority==0 }">
    				<li ><a href="jianli.jsp" rel="nofollow">我的简历</a></li>
    				<li><a rel="nofollow" href="rmanage?op=queryByCid&uid=${loginname.id }&rstate=1">我投递的职位</a></li>
    			</c:if>
    			<c:if test="${loginname.authority==1 }">
    				<li ><a href="create.jsp" rel="nofollow">发布职位</a></li>
    				<li><a rel="nofollow" href="rmanage?op=queryByCid&cid=${cid }&rstate=1">简历管理</a></li>
    			</c:if>
	    		    		</ul>
        	            <dl class="collapsible_menu">
            	<dt>
           			<c:if test="${loginname.authority==0 }">
    				<span>${loginname.uname }&nbsp;</span> 
    			</c:if>
           		<c:if test="${loginname.authority==1 }">
    				<span>${cname }&nbsp;</span> 
    			</c:if>
                ...
                <c:if test="${loginname.authority==0 }">
                	 ...
                	 <dd class="btm"><a href="index.jsp">我要找工作</a></dd>
                	 <dd ><a href="rmanage?op=queryByCid&uid=${loginname.id }">我投递的职位 </a></dd>
                </c:if>    
                <c:if test="${loginname.authority==1 }">
                     <dd><a href="rmanage?op=queryByCid&cid=${cid }&rstate=1">我收到的简历</a></dd> 
                	<dd><a href="position?op=list">我发布的职位</a></dd>
                	<dd class="btm"><a href="subscribe.html">我的订阅</a></dd>
                	<dd><a href="create.jsp">我要招人</a></dd>
                </c:if>   
                <dd><a href="accountBind.jsp">帐号设置</a></dd>
                <dd class="logout"><a rel="nofollow" href="register?op=kill">退出</a></dd>
            </dl>
                                </div>
    </div><!-- end #header -->
    <div id="container">
        ...
        <div class="content">
        	<div id="search_box">
		<form id="searchForm1" name="searchForm" action="SearchList" method="get">
        <ul id="searchType">
...
<dl class="hotSearch">
	<dt>热门搜索：</dt>
	<dd><a href="SearchList?spc=1&kd=Java&labelWords=label&city=全国">Java</a></dd>
	<dd><a href="SearchList?spc=1&kd=PHP&labelWords=label&city=全国">PHP</a></dd>
	<dd><a href="SearchList?spc=1&kd=Android&labelWords=label&city=全国">Android</a></dd>
	<dd><a href="SearchList?spc=1&kd=iOS&labelWords=label&city=全国">iOS</a></dd>
	<dd><a href="SearchList?spc=1&kd=前端&labelWords=label&city=全国">前端</a></dd>
	<dd><a href="SearchList?spc=1&kd=产品经理&labelWords=label&city=全国">产品经理</a></dd>
	<dd><a href="SearchList?spc=1&kd=UI&labelWords=label&city=全国">UI</a></dd>
	<dd><a href="SearchList?spc=1&kd=运营&labelWords=label&city=全国">运营</a></dd>
	<dd><a href="SearchList?spc=1&kd=BD&labelWords=label&city=全国">BD</a></dd>
	<dd><a href="SearchList?spc=1&kd=实习&labelWords=label&city=全国">实习</a></dd>
            	...
            	<a href="javascript:;">我知道了</a>
            	            ...
            	            <c:forEach items="${plist }" var="p">
	                    		   <li class="odd clearfix">
			                			                				                	<div class="hot_pos_l">
			                        <div class="mb10">
			                            <a href="toudi?pid=${p.pid }&cid=${p.company.cid }" title="${p.pname }" target="_blank">${p.pname }</a> 
			                            &nbsp;
			                            <span class="c9">${p.pcity }</span>
			                            			                        </div>
			                        <span><em class="c7">月薪：</em>${p.pfromsalary }k-${p.ptosalary }k</span>
			                        <span><em class="c7">经验：</em> ${p.pexperience }</span>
			                        <span><em class="c7">最低学历： </em>${p.pdegree }</span>
			                        <br />
			                        <span><em class="c7">职位诱惑：</em>${p.pgood }</span>
			                        <br />
			                        <span>刚刚发布</span>
			                    </div>
						        ...
						        <div class="hot_pos_r">
			                        <c:if test="${loginname.authority==0 }">
			                        <div class="apply">
			                        <a href="toudi?pid=${p.pid }&cid=${p.company.cid }" target="">投个简历</a>
			                        </div>
			                        </c:if>
			                        <div class="mb10"><a href="toudi?pid=${p.pid }&cid=${p.company.cid }" title="${p.company.cname }" target="_blank">${p.company.cname }</a></div>
			                        <span><em class="c7">领域： </em>${p.company.cfield }</span>
			                        			                        <span><em class="c7">创始人：</em> ${p.company.cinvest }</span>
			                        			                        <br />
			                        <span><em class="c7">阶段： </em>${p.company.cdevelop }</span>
			                        <span><em class="c7">规模： </em>${p.company.clarge }</span>
			                    ...
			                    <c:forEach items="${clist }" var="c">
	                    		                    			                <li class="odd clearfix">
			                			                				                	<div class="hot_pos_l">
			                        <div class="mb10">
			                            <a href="toudi?pid=${c.position.pid }&cid=${c.cid }" title="${c.position.pname }" target="_blank">${c.position.pname }</a> 
			                            &nbsp;
			                            <span class="c9">${c.position.pcity }</span>
			                            			                        </div>
			                        <span><em class="c7">月薪：</em>${c.position.pfromsalary }k-${c.position.ptosalary }k</span>
			                        <span><em class="c7">经验：</em> ${c.position.pexperience }</span>
			                        <span><em class="c7">最低学历： </em>${c.position.pdegree }</span>
			                        <br />
			                        <span><em class="c7">职位诱惑：</em>${c.position.pgood }</span>
			                        <br />
			                        <span>1天前发布</span>
			                    </div>			                   
						        <div class="hot_pos_r">
			                        <div class="apply">
			                        <a href="toudi?pid=${c.position.pid}&cid=${c.cid }" target="">投个简历</a>
			                        				                        </div>
			                        <div class="mb10"><a href="toudi?pid=${c.position.pid}&cid=${c.cid }" title="${c.cname }" target="_blank">${c.cname }</a></div>
			                        <span><em class="c7">领域： </em>${c.cfield }</span>
			                        <span><em class="c7">创始人：</em> ${c.cinvest }</span>
			                        			                        <br />
			                        <span><em class="c7">阶段： </em>${c.cdevelop }</span>
			                        <span><em class="c7">规模： </em>${c.clarge }</span>
			                        <ul class="companyTags reset">
			                            			                            				                            		<li>五险一金</li>
			                            				                            			                            				                            		<li>股票期权</li>              				                            			                            				                            		<li>年度旅游</li>
			                            				                            			                        </ul>
			                    </div>
			                    </li>
			                    </c:forEach>
...
function editForm(inputId,inputValue){
	...
	if(kw == ''){
		$('#searchForm1').attr('action','SearchList?op='+b).submit();	
	}else{
		kw = kw.replace(re,'井');
		kw = kw.replace(r,'。');
		$('#searchForm1').attr('action','SearchList?op='+b).submit();
